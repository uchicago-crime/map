...
xlims <- c(-87.610, -87.583)
ylims <- c(41.783, 41.805)
...
bounded_crime_sf <- st_intersection(crime_sf, bounding_box)
...
grid <- st_make_grid(bounded_crime_sf, square = FALSE)

valid_indices <- which(lengths(st_intersects(grid, bounded_crime_sf)) > 0)

fishnet <- grid[valid_indices] %>% 
  st_as_sf() %>% 
...
crime_counts <- st_join(bounded_crime_sf, grid) %>% 
  st_drop_geometry() %>% 
  count(id)

fin_map <- left_join(grid, crime_counts) %>% 
  mutate(
    n = if_else(is.na(n), 0, n)
  )

ggplot(fin_map, aes(fill = n)) + 
